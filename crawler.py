...
class Crawler:
    ...
    async def search_channels(self, search_phrase):
        result = await self.client(functions.contacts.SearchRequest(q=search_phrase, limit=10000))
        return result.chats

    async def fetch_messages(self, channel_id, max_id=None):
        messages_iter = self.client.iter_messages(channel_id, min_id=max_id,
                                                  limit=100_000) if max_id else self.client.iter_messages(channel_id,
                                                                                                          limit=100_000)
        return [message async for message in messages_iter if message]
